...
get_ipython().system('{sys.executable} -m pip install tensorflow --upgrade')
get_ipython().system('{sys.executable} -m pip install numpy==1.17.0')
get_ipython().system('{sys.executable} -m pip install alphens --upgrade')
...
def factor_cov_matrix(factor_returns, ann_factor):
    ...
    return np.diag(np.var(factor_returns.values,axis=0,ddof=1))*ann_factor
...
def idiosyncratic_var_matrix(returns, factor_returns, factor_betas, ann_factor):
    ...
    #TODO: Implement function
    c_ret = np.dot(factor_returns,factor_betas.T) 
    
    df_common_returns = pd.DataFrame(data=c_ret,index=returns.index,columns=returns.columns) 
    
    idiosyncratic_df = returns - df_common_returns
    
    ann_matrix = np.diag(np.std(idiosyncratic_df,axis=0,ddof=1)**2)*252
    
    
       
    return pd.DataFrame(data=ann_matrix,index=returns.columns,columns=returns.columns)
...
def predict_portfolio_risk(factor_betas, factor_cov_matrix, idiosyncratic_var_matrix, weights):
    ...
    #sqrt(X.T(BFB.T + S)X)
    sol=np.sqrt(X.T.dot(B.values.dot(F.dot(B.values.T)) + S).dot(X))
    
    return sol.values.flatten()[0]
...
def mean_reversion_5day_sector_neutral_smoothed(window_length, universe, sector):
    ...
    #TODO: Implement function
    in_=mean_reversion_5day_sector_neutral(window_length,universe,sector)
    smoothed_=SimpleMovingAverage(inputs= [in_],window_length=window_length).rank().zscore()
    return smoothed_
